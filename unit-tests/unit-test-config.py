...
def usage():
    ...
    print(f'Syntax: {ourname} [options] <dir> <build-dir>')
...
def generate_cmake( builddir, testdir, testname, filelist, custom_main ):
    makefile = f'{builddir}/{testdir}/CMakeLists.txt'
...
source_group( "Common Files" FILES ${CATCH_FILES} ''' + dir + '''/test.cpp''' )
        if not custom_main:
            handle.write(f' {dir}/unit-test-default-main.cpp')
        handle.write( ''' )
...
def find_include( include, relative_to ):
    ...
    if include:
        if not os.path.isabs( include ):
            include = os.path.normpath(f'{relative_to}/{include}')
...
def find_include_in_dirs( include ):
    ...
    for include_dir in standard_include_dirs:
        if path := find_include(include, include_dir):
            return path
...
def find_includes( filepath, filelist = set() ):
    ...
    try:
        ...
        for include_line in file.grep( r'^\s*#\s*include\s+("(.*)"|<(.*)>)\s*$', filepath ):
            ...
            if (
                include := find_include(m.group(2), filedir)
                or find_include_in_dirs(m.group(2))
                or find_include_in_dirs(m.group(3))
            ):
